...
function install(){
        ...
        if [ -e "$agent_install_path" ]; then
                ...
                sudo chmod 755 $agent_file
                sudo cp -r $agent_file $agent_install_path
        ...
        if [ -e "$manager_install_path" ]; then
                echo "[+] $manager_fisle already installed.  ->  $manager_install_path"
        else
                sudo chmod 755 $manager_file
                sudo cp -r $manager_file $manager_install_path
